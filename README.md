...
```
curl -Lo RED++.zip https://github.com/SysAdminDoc/REDplusplus/releases/latest/download/RED++_v1.6.0.zip
tar -xf RED++.zip
RED+.exe
```
...
- No runtime to install — the release is a self-contained single-file build that bundles .NET 10. Just unzip and run.
...
```
gh attestation verify RED++_v1.6.0.zip -R SysAdminDoc/REDplusplus
```
...
| `-exclude <name>` | Skip directories matching `<name>` for this run (repeatable, composable with config filters). |
| `-protect <name>` | Prevent deletion of directories matching `<name>` for this run (repeatable). |
