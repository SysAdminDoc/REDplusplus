...
### Modern UI
- Tune the results grid for very large scans: recycle list containers instead of recreating one per scroll, and defer realization until the scrollbar thumb is released, so a whole-volume scan returning hundreds of thousands of rows scrolls smoothly with bounded memory.
...
### Reliability
- Version the settings schema and migrate on load. `RED+.cfg` now carries a `schemaVersion`; an older-schema file is upgraded in place — with the original backed up to `RED+.cfg.v<n>.bak` — instead of silently dropping a renamed or restructured setting, and a file from a newer RED++ is left untouched (its unknown fields are ignored, and logged). Config saves are now atomic (stage to a temp file, then replace), so a crash or power loss mid-write can no longer truncate the settings file.
- Add global unhandled-exception handling. An otherwise-fatal error now writes a self-contained crash report (`RED++.crash-<timestamp>.txt` next to the log — exception, OS, runtime, and app version only, never file contents) plus a `RED++.log` pointer, shows a calm dialog in the GUI (or writes to stderr when headless), and exits non-zero — instead of crashing with the default OS dialog and no record (which violated the project's own GUI error-handling convention). Wired across the WPF `DispatcherUnhandledException`, the WinForms `-classic` `ThreadException` (with `SetUnhandledExceptionMode(CatchException)`), the any-thread `AppDomain.UnhandledException`, and the faulted-`Task` `UnobservedTaskException` paths. No telemetry/network — the user attaches the file to a GitHub issue.
...
- Harden the USN/MFT record parser to clamp a caller-reported buffer length to the actual buffer size, so a corrupt or hostile `FSCTL_ENUM_USN_DATA` result that over-reports its byte count can no longer drive an out-of-bounds read. (Found by the new parser fuzz tests.)
...
### Developer / CI
- Add randomized + mutation-based fuzz/property tests for the three parsers that read untrusted on-disk bytes — USN/MFT records, gettext `.mo` catalogs, and undo-manifest JSON — feeding thousands of pure-random and structured-mutation inputs per run and asserting each parser's bounded contract (no unexpected unhandled exception), plus a gettext save→load round-trip property. Dependency-free and seeded for reproducibility; they caught the MFT over-length read fixed above.
